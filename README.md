...
본 프로젝트는 비용 및 리소스 효율성을 위해 **로컬 개발 환경(MacBook)과 프로덕션 환경(AWS)을 분리하는 투트랙(Two-track) 전략**을 사용합니다. 또한, 특정 AI 모델에 종속되지 않는 **Multi-LLM 아키텍처**를 지향합니다.
...
* **AI Backend & Multi-LLM Strategy**:

  * `Python (FastAPI)`: LangChain 등 무거운 프레임워크를 배제하고, 순수 Python 기반의 **Adapter/Factory 패턴**을 구현.

  * `LLM Provider Switch`: 환경변수(`LLM_PROVIDER`)를 통해 로컬 OSS LLM과 클라우드 LLM을 즉각 교체 및 성능 비교 가능하도록 설계.

    * **Primary (OSS)**: `Ollama` (Llama3 8B / Phi3) - 보안 및 비용 최적화

    * **Evaluation/Cloud**: `Gemini 1.5 Pro` - 고성능 추론 및 벤치마크용
...
* **AI Inference**: AWS 환경 배포 시, GPU 인스턴스 제약을 피하기 위해 외부 API(Gemini Pro 등)로 스위칭하여 구동.
...
* **성공 기준**: "결제 API가 느리다"고 질문 시, AI(Gemini/Ollama)가 Tool Calling으로 Prometheus와 Loki를 조회하고, `runbook.md`를 참조하여 서킷 브레이커 발동 명령어를 추천함.

### Phase 3: Web UI 통합 및 AWS 포팅

* **목표**: 포트폴리오 시각화용 Web UI 완성 및 클라우드 환경 배포.

* **성공 기준**: Terraform 코드로 AWS 인프라(일반 CPU 인스턴스)를 생성하고, `Streamlit Web UI`를 통해 K8s 상태 조회 및 AI 조치 가이드 정상 동작 확인.
